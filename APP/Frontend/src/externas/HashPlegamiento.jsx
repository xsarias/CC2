// src/externas/HashPlegamiento.jsx
import React, { useState, useEffect } from "react";
import "../App.css";
import "../internas/IngresarDatos.css";
import "./externas.css";

export default function HashPlegamiento({ onBack }) {
  ...
  // -----------------------------------------------------
  // HASH PLEGAMIENTO (segmentación simple, izquierda→derecha)
  // -----------------------------------------------------
  const hash = (c) => {
    // c: clave numérica como string o number
    const k = String(c);
    const n = Number(tamano) || 1;

    // número de dígitos a usar por segmento según n
    const digitsN = String(n).length; // ejemplo n=100 -> 3
    const segmentSize = Math.max(1, digitsN - 1); // n=100 -> 2

    const partes = [];
    for (let i = 0; i < k.length; i += segmentSize) {
      const slice = k.slice(i, i + segmentSize);
      partes.push(Number(slice) || 0);
    }

    const suma = partes.reduce((acc, v) => acc + v, 0);

    let pos = Number(suma) + 1;
  ...
  const insertar = async () => {
    ...
    // copia profunda del area principal
    const principalCopia = JSON.parse(JSON.stringify(areaPrincipal));
  ...
  const eliminar = () => {
    ...
    // principal
    const copia = JSON.parse(JSON.stringify(areaPrincipal));
    ...
    // cubetas
    if (metodoColision === "cubetas") {
      const cub = JSON.parse(JSON.stringify(cubetas || []));
  ...
  return (
    <div className="contenedor-hash contenedor-secuencial">
      <h3>Hash Plegamiento — Método Plegamiento (segmentos simples)</h3>
